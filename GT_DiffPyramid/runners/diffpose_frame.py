...
def weighted_loss(args, device):#l1-loss
    args.norm = 0.025
    assert 0 <= args.norm <= 1, "args.norm must be between 0 and 1."

    mse_loss = nn.MSELoss(reduction='mean').to(device)
    l1_loss = nn.L1Loss(reduction='mean').to(device)

    def combined_loss(preds, targets):
        mse_part = mse_loss(preds, targets)
        l1_part = l1_loss(preds, targets)
        total_loss = (1 - args.norm) * mse_part + args.norm * l1_part
        return total_loss
...
class Diffpose(object):
    ...
    def create_diffusion_model(self, model_path = None):
        ...
        diff_params = count_model_parameters(self.model_diff)
        self.parameters_cnt += diff_params
        print(f"Number of parameters in the diffusion model: {diff_params}")
    ...
    def create_pose_model(self, model_path = None):
        ...
        pose_params = count_model_parameters(self.model_pose)
        self.parameters_cnt += pose_params
        print(f"Number of parameters in the pose model: {pose_params}")
        print(f"Number of SUM parameters: {self.parameters_cnt}")
    ...
    def train(self):
        ...
        criterion = weighted_loss(args, self.device)
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (targets_uvxyz, targets_noise_scale, _, targets_3d, _, _) in enumerate(data_loader):
                ...
                loss_diff = criterion(output_noise,e)
    ...
    def train_pose(self):
        ...
        lr_now, decay, gamma = self.config.optim.lr, self.config.optim.decay, self.config.optim.lr_gamma
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (targets_uvxyz, targets_noise_scale, input_2d, targets_3d, input_action, camera_para) in enumerate(data_loader):
                step += 1
                num_poses = targets_3d.size(0)

                # to cuda
                input_2d, targets_3d = \
                    input_2d.to(self.device), targets_3d.to(self.device)

                # build uvxyz
                output_xyz = self.model_pose(input_2d, src_mask)
                optimizer.zero_grad()
                loss_3d_pos = criterion(output_xyz, targets_3d)
                loss_3d_pos.backward()
                torch.nn.utils.clip_grad_norm_(
                    self.model_pose.parameters(), config.optim.grad_clip)
                optimizer.step()

                epoch_loss_3d_pos.update(loss_3d_pos.item(), num_poses)

                if i % 1000 == 0 and i != 0:
                    logging.info('| Epoch{:0>4d}: {:0>4d}/{:0>4d} | Step {:0>6d} | Loss: {:.6f} |' \
                                 .format(epoch, i + 1, len(data_loader), step, epoch_loss_3d_pos.avg))

            if epoch % decay == 0:
                lr_now = lr_decay(optimizer, epoch, lr_now, decay, gamma)

            if epoch % 1 == 0:
                states = [
                    self.model_pose.state_dict(),
                    optimizer.state_dict(),
                    epoch,
                    step,
                ]
                ...
                p1, p2 = self.test_pose(is_train=True)
    ...
    def test_pose(self, is_train=False):
        cudnn.benchmark = True
