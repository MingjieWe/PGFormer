...
from models.PGFormer import PGFormer, adj_mx_from_edges
...
def main(args):
    ...
    model_pos = PGFormer(adj=adj.to(device), hid_dim=args.dim_model, coords_dim=(2, 3), n_pts=16,
                          num_layers=args.n_layer, n_head=args.n_head, dropout=args.dropout).to(device)
    ...
    if args.resume or args.evaluate:
        ...
        ckpt_dir_path = path.join(args.checkpoint, 'PGFormer-' + args.keypoints,
    ...
    if args.evaluate:
        ...
        for i, action in enumerate(action_filter):
            ...
            action_wise_error.append((action, errors_p1[i], errors_p2[i]))
        ...
        for action_error in action_wise_error:
            print("{0:<12} ".format(action_error[0]), end="")
            print("{0:>6.2f} {1:>10.2f}".format(action_error[1], action_error[2]))
        exit(0)
...
def evaluate(data_loader, model_pos, device):
    ...
    for i, (targets_3d, inputs_2d, _) in enumerate(data_loader):
        ...
        bar.suffix = '({batch}/{size}) Data: {data:.6f}s | Batch: {bt:.3f}s | Total: {ttl:} | ETA: {eta:} ' \
            ...
            .format(batch=i + 1, size=len(data_loader), data=data_time, bt=batch_time.avg,
                    ttl=bar.elapsed_td, eta=bar.eta_td, e1=epoch_loss_3d_pos.avg,
                    e2=epoch_loss_3d_pos_procrustes.avg)
        bar.next()

    bar.finish()
    print('Data: {data:.6f}s | Batch: {bt:.3f}s '.format(data=data_time, bt=batch_time.avg))
