...
def get_resolution(filename):
    ...
    try:
        ...
        for line in pipe.stdout:
            width, height = line.decode().strip().split(',')
    finally:
        pipe.stdout.close()
...
def read_video_getframe(filename,frameNum):
    import cv2
    vidcap = cv2.VideoCapture(filename)
    # 获取帧数
    framecnt = vidcap.get(7)

    if frameNum > framecnt:
        frameNum = 1
    print(f"frameNum = {frameNum} | last framecnt = {framecnt}")
